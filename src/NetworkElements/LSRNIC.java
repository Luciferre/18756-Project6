...
public class LSRNIC {
    ...
    private void runRED(Packet currentPacket) {
        ...
        if (outputBuffer.size() > startDropAt) {
            if (outputBuffer.size() >= maximumBuffer) {
                dropProbability = 1.0;
                packetDropped = true;
            } else {
                dropProbability = (double) (outputBuffer.size() - startDropAt) / (maximumBuffer - startDropAt);
    ...
    public int getLinkDestAddr() {
        return link.getDestAddr(this);
    }

    public LSR getLinkDestLSR() {
        return link.getDestLSR(this);
    }
